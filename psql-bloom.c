/*-------------------------------------------------------------------------
 ...
 #define FLEXIBLE_ARRAY_MEMBER 100
 ...
 static int  my_bloom_power(unsigned long target_bitset_bits);
 static int  optimal_k(unsigned long bitset_bits, long total_elems);
 static void k_hashes(bloom_filter *filter, long *hashes, unsigned char *elem,
                      size_t len);
 static inline long mod_m(long a, unsigned long m);
  ...
  */
 bloom_filter *
 bloom_create(long total_elems, int bloom_work_mem, unsigned long seed)
 {
     bloom_filter *filter;
     int         bloom_power;
     unsigned long      bitset_bytes;
     unsigned long      bitset_bits;
      ...
      */
     bitset_bytes = Min(bloom_work_mem * unsigned longCONST(1024), total_elems * 2);
     bitset_bytes = Max(1024 * 1024, bitset_bytes);
 
     /*
      * Size in bits should be the highest power of two <= target.  bitset_bits
      * is unsigned long because PG_long_MAX is 2^32 - 1, not 2^32
      */
     bloom_power = my_bloom_power(bitset_bytes * BITS_PER_BYTE);
     bitset_bits = unsigned longCONST(1) << bloom_power;
 ...
 bloom_prop_bits_set(bloom_filter *filter)
 {
     ...
     unsigned long      bits_set = pg_popcount((char *) filter->bitset, bitset_bytes);
  ...
  */
 static int
 my_bloom_power(unsigned long target_bitset_bits)
 {
     int         bloom_power = -1;
  ...
  */
 static int
 optimal_k(unsigned long bitset_bits, long total_elems)
 {
  ...
  */
 static void
 k_hashes(bloom_filter *filter, long *hashes, unsigned char *elem, size_t len)
 {
     unsigned long      hash;
     long      x,
                 y;
     ...
     hash = DatumGetunsigned long(hash_any_extended(elem, len, filter->seed));
     x = (long) hash;
     y = (long) (hash >> 32);
     m = filter->m;
  ...
  */
 static inline long
 mod_m(long val, unsigned long m)
 {
     Assert(m <= PG_long_MAX + unsigned longCONST(1));
     Assert(((m - 1) & m) == 0);
